...
const defaultMediaItems = [
  {
    ...
    clipId: 'clip-goey1',
    type: 'image',
    figureClassName: defaultFigureClasses['clip-goey1'],
    label: 'Stellaris HQ',
...
const ClippedGoeyGallery = React.forwardRef(({ mediaItems, className, ...props }, ref) => {
  const itemsToRender = mediaItems || defaultMediaItems;

  return (
    <>
      {/* SVG clip paths — hidden off-screen */}
      ...
      >
        {itemsToRender.map((item, index) => (
          <figure key={index} className={`relative ${item.figureClassName || defaultFigureClasses[item.clipId]}`}>
            <div style={{ clipPath: `url(#${item.clipId})` }}>
              {item.type === 'image' ? (
                <img
                  src={item.src}
                  alt={item.alt}
                  className="transition-all duration-500 align-bottom object-cover aspect-square group-hover:scale-110 w-full"
                />
              ) : (
                <video
                  autoPlay
            ...
            {(item.label || item.category) && (
              <figcaption className="mt-4 text-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                {item.label && <p className="text-sm font-semibold text-white/90 font-[Newsreader]">{item.label}</p>}
                {item.category && <p className="text-[10px] uppercase tracking-widest text-white/60 mt-0.5">{item.category}</p>}
              </figcaption>
            )}
          </figure>
        ))}
